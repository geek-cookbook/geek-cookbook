...
Ha, but we're not done yet! We have exposed a service via our load balancer, we've exposed a route to a service via an Ingress, but let's git rid of that nasty "insecure" message in the browser when using HTTPS...
